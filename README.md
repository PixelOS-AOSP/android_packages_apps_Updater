...
| `net.pixelos.version` | Official-devices branch | OTA, changelog, and certified-properties URLs |
...
| `config_certifiedPropsUpdaterEnabled` | `true` | Shows the certified-properties updater |
...
## Certified-properties updates

When `config_certifiedPropsUpdaterEnabled` is true, preferences can check and install the optional
package `net.pixelos.certifiedprops.overlay`. The manifest URL is:

```text
https://raw.githubusercontent.com/PixelOS-AOSP/official_devices/{branch}/API/updater/v2/certified-props.json
```
...
```json
{
  "package_name": "net.pixelos.certifiedprops.overlay",
  "version_code": 42,
  "sha256": "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
  "size": 123456,
  "url": "https://downloads.example.org/certified-props.apk",
  "signing_certificate_sha256": "30887e0e242a6e5ea4a477881e231d96822dc274d8d89f0abf778199aabc5747"
}
```

This is a security-sensitive, independently versioned channel. Updater accepts only the embedded
package name and signing-certificate SHA-256, and then verifies the manifest, HTTPS URL, 16 MiB
download bound, exact size, file SHA-256, archive package name, archive version code, archive
signer, and installed-package signer before submitting a platform `PackageInstaller` session. The
manifest is capped at 64 KiB. A network attacker or compromised JSON alone therefore cannot change
the trusted package or signing key.

Generate and validate the manifest with:

```sh
tools/pixelos_feed.py generate-certified-props certified-props.apk \
  --url https://downloads.example.org/certified-props.apk \
  --version-code 42 \
  --signer-sha256 30887e0e242a6e5ea4a477881e231d96822dc274d8d89f0abf778199aabc5747 \
  --output certified-props.json

tools/pixelos_feed.py validate-certified-props certified-props.json \
  --apk certified-props.apk
```

The standard-library tool cannot inspect Android signing blocks. Derive and review the signer
digest with Android signing tooling, then keep the manifest digest equal to the pinned
`certified_props_signer_sha256` string resource. A signing-key rotation requires a reviewed app
update that changes the embedded pin; changing only the server manifest is deliberately
insufficient.
